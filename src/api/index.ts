...
import { GetTeeTimesResponse, TeeTime, Cart, CartItem, Reservation } from "../models";
...
export const addTeeTimeToCart = async (teetime: TeeTime) => {
    const url =
        `https://phx-api-be-east-1b.kenna.io/shopping-cart/${getState('cartId')}/cart-item`;
...
export const updateCartWithReservationId = async (reservationId: string, cartItem: CartItem) => {
    const url = `https://phx-api-be-east-1b.kenna.io/shopping-cart/${getState('cartId')}/cart-item/${cartItem.id}`;
...
export const deleteCartItem = async (cartItemId: string, removeChildren: boolean) => {
    const url = `https://phx-api-be-east-1b.kenna.io/shopping-cart/${getState('cartId')}/cart-item/${cartItemId}?removeChildren=${removeChildren.toString()}`;
...
export const getCart = async (sessionToken: string) => {
    const url = `https://phx-api-be-east-1b.kenna.io/shopping-cart/${getState('cartId')}`;
...
export const generateReservation = async (teeTime: TeeTime, cart: Cart): Promise<Reservation> => {
